...
export function NavMain({ items = [], groups = [], role }: { items?: NavItem[]; groups?: NavGroup[]; role?: string }) {
    ...
    useEffect(() => {
        ...
        if (!hasInitialized.current || lastUrl.current !== currentUrl) {
            ...
            items.forEach((item) => {
                if (item.subNav) {
                    newOpenSections[item.title] = currentUrl.startsWith(item.href as string);
                }
            });
        ...
        return navItems.map((item) => (
            <SidebarMenuItem key={item.title}>
                {item.subNav ? (
                        ...
                        <TooltipProvider>
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <SidebarMenuButton
                                        isActive={currentUrl.startsWith(item.href as string) || item.subNav.some(sub => currentUrl === sub.href)}
                                        className="w-full"
                                    >
                    ...
                    <SidebarMenuButton asChild isActive={currentUrl.startsWith(item.href as string)} tooltip={{ children: item.title }}>
